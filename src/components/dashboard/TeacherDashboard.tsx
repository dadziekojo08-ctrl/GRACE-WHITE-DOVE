...
import { getSubjectBadgeColor, getSlotStartMinutes } from '../../utils/timetableUtils';
...
export const TeacherDashboard: React.FC<{ initialTab?: TeacherDashboardTab }> = ({ initialTab = 'overview' }) => {
  ...
  // Calculate default day of week (Monday to Friday) based on today
  const defaultScheduleDay = useMemo<TimetableEntry['day']>(() => {
    const dayNames: TimetableEntry['day'][] = ['Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday'];
    const currentDayNum = new Date().getDay(); // 0 Sun, 1 Mon, 2 Tue, 3 Wed, 4 Thu, 5 Fri, 6 Sat
    if (currentDayNum >= 1 && currentDayNum <= 5) {
      return dayNames[currentDayNum - 1];
    }
    return 'Monday';
  }, []);
  const [selectedScheduleDay, setSelectedScheduleDay] = useState<TimetableEntry['day']>(defaultScheduleDay);
  ...
  // Digital ID Card & Quick Photo Upload Modals for Teachers
  const [isIdCardModalOpen, setIsIdCardModalOpen] = useState(false);
  const [selectedIdStudent, setSelectedIdStudent] = useState<Student | null>(null);
  const [photoUploadStudent, setPhotoUploadStudent] = useState<Student | null>(null);
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-200">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            <div className="lg:col-span-2 bg-white rounded-2xl p-5 border border-slate-200 shadow-xs">
              ...
              <div className="flex items-center gap-1.5 overflow-x-auto pb-2 mb-3 border-b border-slate-100">
                {(['Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday'] as TimetableEntry['day'][]).map((d) => {
                  const dayLessonsCount = timetable.filter(t => {
                    const matchDay = (t.day || '').trim().toLowerCase() === d.toLowerCase();
                  ...
                  const isToday = defaultScheduleDay === d;
              ...
              <div>
                {(() => {
                  const dayLessons = timetable.filter(t => {
                    const matchDay = (t.day || '').trim().toLowerCase() === selectedScheduleDay.toLowerCase();
                    if (!matchDay) return false;
                    if (teacherAssignedClass && t.className === teacherAssignedClass) return true;
                    if (teacherName && (t.teacherName || '').toLowerCase().includes(teacherName.toLowerCase())) return true;
                    return false;
                  }).sort((a, b) => getSlotStartMinutes(a.timeSlot) - getSlotStartMinutes(b.timeSlot));

                  if (dayLessons.length === 0) {
                    return (
                      <div className="p-8 text-center rounded-xl bg-slate-50 border border-dashed border-slate-200">
                        <CalendarDays className="w-8 h-8 text-slate-300 mx-auto mb-2" />
                        <p className="text-xs font-bold text-slate-700">No Lessons Scheduled for {selectedScheduleDay}</p>
                        <p className="text-[11px] text-slate-400 mt-0.5">
                          You can set up lesson timings, start times, and subjects directly for {selectedScheduleDay}.
                        </p>
                        <button
                          onClick={() => setCurrentTab('timetable')}
                        ...
                        >
                          ...
                          Add {selectedScheduleDay} Lesson
                        </button>
                      </div>
                    );
                  }
                    ...
                    <div className="space-y-2.5">
                      {dayLessons.map((slot, idx) => (
                        <div
                          ...
                          title={`Click to view or edit ${slot.subject} on ${slot.day}`}
                        >
                          <div className="flex items-center gap-3">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
              ...
              <div className="flex flex-wrap items-center gap-2">
                ...
                <button
                  ...
                  className="bg-amber-50 hover:bg-amber-100 text-amber-950 border border-amber-300 font-bold px-3 py-2 rounded-xl text-xs flex items-center gap-1.5 shadow-2xs transition-all cursor-pointer"
                  title="Print Digital ID Cards for class pupils"
                >
                    ...
                    <tr key={std.id} className="hover:bg-slate-50/80 transition-colors">
                      ...
                      <td className="py-3 px-3">
                        <div className="flex items-center gap-2.5">
                          <div
                            ...
                            onClick={() => setPhotoUploadStudent(std)}
                            title="Click to upload or snap ID card photo"
                          >
                            <img
                              src={std.photoUrl}
                              alt=""
                              className="w-8 h-8 rounded-full object-cover border border-slate-200 group-hover/avatar:ring-2 group-hover/avatar:ring-emerald-700 transition-all"
                            />
                      ...
                      <td className="py-3 px-3 text-right">
                        <div className="flex items-center justify-end gap-1.5">
                          <button
                            onClick={() => setPhotoUploadStudent(std)}
                            className="p-1.5 text-emerald-800 hover:bg-emerald-50 rounded-lg cursor-pointer transition-colors"
                            title="Upload or Snap ID Card Photo"
                          >
                            <Camera className="w-3.5 h-3.5" />
                          </button>
                          <button
                            ...
                            className="p-1.5 text-amber-800 hover:bg-amber-50 rounded-lg cursor-pointer transition-colors"
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-xs">
          <div className="bg-white rounded-2xl w-full max-w-xl max-h-[90vh] overflow-y-auto p-6 shadow-2xl border border-slate-200">
            ...
            <form onSubmit={handleAdmitStudentSubmit} className="mt-4 space-y-4">
              {/* Pupil Passport Photo Upload */}
              <div className="bg-slate-50 p-3.5 rounded-xl border border-slate-200">
                <StudentPhotoUpload
                  currentPhotoUrl={admitFormData.photoUrl}
                  studentName={`${admitFormData.firstName} ${admitFormData.lastName}`.trim() || 'Pupil'}
                  onPhotoChange={(url) => setAdmitFormData((prev) => ({ ...prev, photoUrl: url }))}
                  label="Student Passport Photo (for ID Card Printing)"
                  helperText="Upload or snap pupil photo with device camera. Auto-crops to high-resolution badge dimensions."
                />
              </div>
      ...
      {/* Teacher Digital ID Card Generator Modal */}
      {isIdCardModalOpen && (
        <DigitalIdCardGenerator
          isOpen={isIdCardModalOpen}
          initialStudent={selectedIdStudent}
          initialClass={teacherAssignedClass || teacherAllowedClasses[0]?.name}
          onClose={() => {
      ...
      {/* Teacher Quick Photo Upload Modal */}
      {photoUploadStudent && (
        <QuickPhotoUploadModal
          student={photoUploadStudent}
          isOpen={!!photoUploadStudent}
          onClose={() => setPhotoUploadStudent(null)}
          onOpenIdCard={(std) => {
            setSelectedIdStudent(std);
            setIsIdCardModalOpen(true);
          }}
